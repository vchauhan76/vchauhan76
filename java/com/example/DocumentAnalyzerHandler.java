...
public class DocumentAnalyzerHandler implements RequestHandler<S3Event, String> {
	...
	private final static String KNOWLEDGE_BASE_ID = "IIJLY83M5H";
	private String knowledgeBaseText;
	private ReviewPromt reviewPromt;
	...
	public DocumentAnalyzerHandler() {
		...
		this.bedrockAgentClient = BedrockAgentRuntimeClient.builder().region(Region.US_EAST_1).build();
	...
	public String handleRequest(S3Event s3Event, Context context) {
		...
		try {
			...
			extractedText = analyzeDocument(documentBytes, bucket, key);

			context.getLogger().log("[handler]Extracted Text from s3..:" + extractedText);

			knowledgeBaseRetriever = new KnowledgeBaseRetriever(bedrockAgentClient, context);

			knowledgeBaseText = knowledgeBaseRetriever.retrieve(
					"can you provide information based on cost optimization",
					KNOWLEDGE_BASE_ID); // TODO enhance with lense filter.

			context.getLogger().log("[handler]Knowledge Base text context..:" + knowledgeBaseText);

			reviewPromt = new ReviewPromt(bedrockClient, context);

			String prompt = reviewPromt.createReviewPrompt(extractedText, knowledgeBaseText,
					"FinancialServicesIndustryLense", "CostOptimizationPillar",
					"is this architecure fit for finance industry ?");

			String reviewResult = reviewPromt.invokeBedrockModel(prompt);
	...
	private String analyzeDocumentAsync(Document document, String bucket, String key) {
		...
		NotificationChannel channel = NotificationChannel.builder()
				.snsTopicArn("arn:aws:sns:us-east-1:361769561116:AmazonTextractHack")
				.roleArn("arn:aws:iam::361769561116:role/TextractRole").build();
		...
		try {
            ...
            while (attempts < 6000) {
            	GetDocumentTextDetectionRequest getRequest =
            	   ...
            	   GetDocumentTextDetectionResponse result1; 
           		 result1 = textractClient.getDocumentTextDetection(getRequest);
            	
           		 String status = result1.jobStatus().toString();
            	
           		if (status.equals(JobStatus.SUCCEEDED.toString())) {
           			context.getLogger().log("Textract job completed successfully");
                    break;
                } else if (status.equals(JobStatus.FAILED.toString())) {
                	context.getLogger().log("Textract job failed with status: {}" + status);
                    break;
                }
                attempts++;
                Thread.sleep(5 * 1000);
            }
            context.getLogger().log("Textract job timed out");
        } catch (Exception e) {
        	context.getLogger().log("Error monitoring Textract job status");
        }
		 ...
		 */
		/*
		 * try { boolean jobComplete = false;
		 * 
		 * 
		 * 
		 * while (!jobComplete) {
		 * 
		 * GetDocumentTextDetectionRequest request =
		 * GetDocumentTextDetectionRequest.builder() .jobId(jobId) .build();
		 * 
		 * result = textractClient.getDocumentTextDetection(request);
		 * 
		 * context.getLogger().log("Job ID inside while loop" + jobId);
		 * 
		 * String status = result.jobStatus().toString();
		 * 
		 * context.getLogger().log("Status for Job ID " + jobId + "is"+ status);
		 * 
		 * if (status.equals("SUCCEEDED")) { jobComplete = true;
		 * context.getLogger().log("Textract job completed successfully"); } else if
		 * (status.equals("FAILED")) { throw new
		 * RuntimeException("Textract job failed: " + result.statusMessage()); } else
		 * if(status.equals("IN_PROGRESS")) {
		 * context.getLogger().log("Textract job sleeping now for 5 seconds...");
		 * //Thread.sleep(5000); // Wait for 1 second before checking again
		 * 
		 * } else { // Add delay before next polling attempt
		 * context.getLogger().log("Textract job sleeping now...");
		 * //Thread.sleep(5000); // Wait for 1 second before checking again } } }
		 * 
		 * catch (InterruptedException e) { Thread.currentThread().interrupt(); throw
		 * new RuntimeException("Job polling was interrupted", e); } catch
		 * (TextractException e) { throw new
		 * RuntimeException("Error checking Textract job status", e); }
		 */
		...
		do {
			...
			context.getLogger().log("Status for...job id:" + jobId + "is" +result.jobStatus());
			pages.add(result);
		...
		for (GetDocumentTextDetectionResponse page : pages) {
			page.blocks().stream().filter(block -> block.blockType() == BlockType.LINE)
